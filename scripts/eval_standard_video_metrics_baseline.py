...
from vh_mvp.train.trainer import resolve_device
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Evaluate standard video metrics for the Conditional ConvLSTM baseline.")
...
def load_baseline_section(path: str) -> dict[str, int]:
...
def build_model_from_config(config_path: str, checkpoint_path: str, device: torch.device) -> tuple[ConditionalConvLSTMBaseline, object]:
...
def build_dataset(cfg, split: str):
    ...
    if cfg.data.kind == "synthetic":
        ...
        seed = cfg.seed if split == "train" else cfg.seed + 100_000
    ...
    *,
    ...
    for batch in loader:
        ...
        if "sample_id" in batch:
            ...
            sample_ids = [f"{processed + idx:06d}" for idx in range(video.size(0))]
        ...
        recon_mse = video_mse_per_sample(out.recon, video)
        recon_psnr = video_psnr_per_sample(out.recon, video)
        recon_ssim = video_ssim_per_sample(out.recon, video)
        future_mse = video_mse_per_sample(out.future, video[:, 1:])
        future_psnr = video_psnr_per_sample(out.future, video[:, 1:])
        future_ssim = video_ssim_per_sample(out.future, video[:, 1:])
        recon_lpips = video_lpips_per_sample(out.recon, video, lpips_model) if lpips_model is not None else None
        future_lpips = (
            video_lpips_per_sample(out.future, video[:, 1:], lpips_model) if lpips_model is not None else None
        )
