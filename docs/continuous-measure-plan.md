...
1. `trajectory_state` 首先承担的是“局部基点近似”角色；
2. 真正需要补上的对象是该基点附近的局部切向子空间；
3. 全局化的关键不是再发明更复杂的 chart，而是让局部切向对象在邻域中具有兼容性。
...
表示 `\mathcal M_T` 上一点。当前实现中，`x` 由以下量拼接后再投影得到：
...
这使它比单纯的均值 summary 更接近“局部基点”而不是静态类别摘要。
